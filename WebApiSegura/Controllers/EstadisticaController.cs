            ...
            try
            {
                    ...
                    SqlConnection(ConfigurationManager.ConnectionStrings["INTERNET_BANKING"].ConnectionString))
                {
                    SqlCommand sqlCommand = new SqlCommand(@"SELECT Codigo, CodigoUsuario, FechaHora,
                                                            Navegador, PlataformaDispositivo, FabricanteDispositivo, Vista, Accion 
                                                            FROM Estadistica WHERE Codigo = @Codigo", sqlConnection);
                    ...
                    while (sqlDataReader.Read())
                    {
                        ...
                        estadistica.FabricanteDispositivo = sqlDataReader.GetString(5);
        ...
        public IHttpActionResult Ingresar(Estadistica estadistica)
        {
            ...
            try
            {
                using (SqlConnection sqlConnection = new
                    SqlConnection(ConfigurationManager.ConnectionStrings["INTERNET_BANKING"].ConnectionString))
                {
                    SqlCommand sqlCommand = new SqlCommand(@"INSERT INTO Estadistica (CodigoUsuario, FechaHora,
                                                            Navegador, PlataformaDispositivo, FabricanteDispositivo, Vista, Accion) VALUES
                                                            (@CodigoUsuario, @FechaHora,
                                                            @Navegador, @PlataformaDispositivo, @FabricanteDispositivo, @Vista, @Accion)", sqlConnection);
        ...
        public IHttpActionResult Actualizar(Estadistica estadistica)
        {
            ...
            try
            {
                using (SqlConnection sqlConnection = new
                    SqlConnection(ConfigurationManager.ConnectionStrings["INTERNET_BANKING"].ConnectionString))
                {
                    SqlCommand sqlCommand = new SqlCommand(@"UPDATE Estadistica SET CodigoUsuario = @CodigoUsuario,
                                                                               FechaHora = @FechaHora,
                                                                               Navegador = @Navegador, 
        ...
        public IHttpActionResult Eliminar(int id)
        {
            ...
            try
            {
                using (SqlConnection sqlConnection = new
                    SqlConnection(ConfigurationManager.ConnectionStrings["INTERNET_BANKING"].ConnectionString))
                {
                    SqlCommand sqlCommand = new SqlCommand(@"DELETE Estadistica WHERE Codigo = @Codigo", sqlConnection);
