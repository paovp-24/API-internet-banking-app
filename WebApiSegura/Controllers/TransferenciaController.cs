    ...
    public class TransferenciaController : ApiController
    {
        ...
        public IHttpActionResult GetId(int id)
        {
            ...
            try
            {
                ...
                {
                    SqlCommand sqlCommand = new SqlCommand(@"SELECT Codigo, CuentaOrigen, CuentaDestino,
                                                            FechaHora, Descripcion, Monto, Estado 
                                                            FROM Transferencia WHERE Codigo = @Codigo", sqlConnection);
        ...
        public IHttpActionResult Ingresar(Transferencia transferencia)
        {
            ...
            try
            {
                using (SqlConnection sqlConnection = new
                    SqlConnection(ConfigurationManager.ConnectionStrings["INTERNET_BANKING"].ConnectionString))
                {
                    SqlCommand sqlCommand = new SqlCommand(@"INSERT INTO Transferencia (CuentaOrigen, CuentaDestino,
                                                            FechaHora, Descripcion, Monto, Estado) VALUES
                                                            (@CuentaOrigen, @CuentaDestino,
                                                            @FechaHora, @Descripcion, @Monto, @Estado)", sqlConnection);
        ...
        public IHttpActionResult Actualizar(Transferencia transferencia)
        {
            ...
            try
            {
                using (SqlConnection sqlConnection = new
                    SqlConnection(ConfigurationManager.ConnectionStrings["INTERNET_BANKING"].ConnectionString))
                {
                    SqlCommand sqlCommand = new SqlCommand(@"UPDATE Transferencia SET CuentaOrigen = @CuentaOrigen,
        ...
        public IHttpActionResult Eliminar(int id)
        {
            ...
            try
            {
                using (SqlConnection sqlConnection = new
                    SqlConnection(ConfigurationManager.ConnectionStrings["INTERNET_BANKING"].ConnectionString))
                {
                    SqlCommand sqlCommand = new SqlCommand(@"DELETE Transferencia WHERE Codigo = @Codigo", sqlConnection);
